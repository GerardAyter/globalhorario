...
export function useCompanies() {
  ...
  async function load(page = 1) {
    ...
    error.value   = null
    try {
      const res = await api.get(`/v1/companies?page=${page}`)
      ...
      error.value = e?.response?.data?.message || 'Error carregant empreses'
    } finally {
      loading.value = false
    }
  }
  ...
  return { companies, loading, saving, error, pagination, load, create, update, remove }
}
