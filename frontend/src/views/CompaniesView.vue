<template>
  <div>
    <div class="flex items-center justify-between mb-5">
      <div>
        <h2 class="text-base font-medium text-gray-900">Empreses</h2>
        <p class="text-sm text-gray-400 mt-0.5">{{ pagination.total }} empreses registrades</p>
      </div>
      <button @click="openCreate" class="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-3 py-1.5 rounded-lg transition-colors">
        <IconPlus class="w-4 h-4" />Nova empresa
      </button>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div v-else-if="companies.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="text-sm text-gray-500">Encara no hi ha empreses</p>
        <button @click="openCreate" class="mt-3 text-sm text-blue-600 hover:underline">Crea la primera empresa</button>
      </div>
      ...
      <div v-else class="divide-y divide-gray-100">
        <div v-for="c in companies" :key="c.id" class="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
          ...
          <div class="flex items-center gap-1.5 w-28 text-sm text-gray-600">
            ...
            <span>{{ c.employees_count }} {{ c.employees_count === 1 ? 'empleat' : 'empleats' }}</span>
          </div>
          ...
          <div class="flex-1 flex items-center flex-wrap gap-1.5 min-w-0">
            ...
            <span v-else class="text-xs text-gray-400 italic">Sense mòduls</span>
          </div>

          <!-- Botons -->
          <div class="flex items-center gap-1 flex-shrink-0">
            <button @click="viewTarget = c" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" title="Veure detalls">
              <IconEye class="w-4 h-4" />
            </button>
            <button @click="openEdit(c)" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Editar">
              <IconEdit class="w-4 h-4" />
            </button>
            <button @click="askDelete(c)" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar">
      ...
      <div v-if="modal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" @click.self="closeModal">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            <h3 class="font-medium text-gray-900">{{ modal.isEdit ? 'Editar empresa' : 'Nova empresa' }}</h3>
          ...
          <form @submit.prevent="submitModal" class="overflow-y-auto flex-1 px-6 py-5 space-y-6">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-[10px] text-gray-400 mt-1">PNG, ICO, SVG. Max 512 KB.</p>
            ...
            <!-- Identificació -->
            <div>
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Identificació</p>
              <div class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Nom <span class="text-red-500">*</span></label>
                    <input v-model="form.name" type="text" placeholder="Nom de l'empresa"
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input v-model="form.tax_id" type="text" placeholder="B12345678"
                           class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div>
                    ...
                    <input v-model="form.country" type="text" placeholder="Espanya"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Adreça de facturació</label>
                  <input v-model="form.adreca_facturacio" type="text" placeholder="Carrer, núm, població, CP"
                         class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
              ...
              <div class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input v-model="form.persona_contacte" type="text" placeholder="Nom i cognoms"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div>
            ...
            <div v-if="availableModules.length">
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Mòduls habilitats</p>
              <div class="space-y-2">
            ...
            <div class="border rounded-xl overflow-hidden">
              <button type="button" @click="form.createAdmin = !form.createAdmin"
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm font-medium text-gray-700">Afegir usuari administrador</span>
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-xs text-gray-400">{{ form.createAdmin ? 'Actiu' : 'Opcional' }}</span>
              ...
              <div v-if="form.createAdmin" class="px-4 py-4 space-y-3 border-t">
                <div>
                  ...
                  <input v-model="form.admin_name" type="text" placeholder="Nom de l'administrador"
                         class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div class="grid grid-cols-2 gap-3">
                  <div>
                  ...
                  <div>
                    ...
                    <input v-model="form.admin_password" type="password" placeholder="Mínim 8 caràcters"
                           class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div class="flex items-center justify-end gap-2 pt-1 border-t">
              ...
              <button type="submit" :disabled="saving"
                ...
                {{ saving ? 'Desant...' : (modal.isEdit ? 'Desar canvis' : 'Crear empresa') }}
              </button>
            </div>
          </form>
        </div>
    ...
    <Teleport to="body">
      <div v-if="viewTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" @click.self="viewTarget = null">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col">
          <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            <h3 class="font-medium text-gray-900">Detalls de l'empresa</h3>
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4">
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
                  ...
                  <span>{{ viewTarget.employees_count }} {{ viewTarget.employees_count === 1 ? 'empleat' : 'empleats' }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="border-t pt-4">
              <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Mòduls actius</p>
              ...
              <p v-else class="text-xs text-gray-400 italic">Sense mòduls assignats</p>
            </div>

            <!-- Favicon -->
            <div v-if="viewTarget.favicon_url" class="border-t pt-4">
              <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Favicon</p>
      ...
      <div v-if="deleteTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" @click.self="deleteTarget = null">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
          <div class="flex items-start gap-3 mb-5">
            ...
            <div>
              <p class="font-medium text-gray-900">Eliminar empresa</p>
              <p class="text-sm text-gray-500 mt-1">Estàs a punt d'eliminar <strong class="text-gray-800">{{ deleteTarget.name }}</strong>. Totes les dades s'eliminaran permanentment.</p>
            </div>
          </div>
...
// ── Mòduls ────────────────────────────────────────────────────────────────────
const ALL_MODULES = [
  { key: 'time_tracking', label: 'Fitxatge',     desc: 'Control horari i entrades/sortides', cls: 'bg-blue-50 text-blue-700' },
  { key: 'documents',     label: 'Documentació', desc: 'Gestió de documents i arxius',       cls: 'bg-purple-50 text-purple-700' },
  { key: 'calendar',      label: 'Calendari',    desc: "Calendari d'empresa i absències",    cls: 'bg-green-50 text-green-700' },
]
...
function activeCompanyModules(c) {
  ...
  return ALL_MODULES.filter(m => flags.includes(m.key))
}
...
function openEdit(c) {
  ...
  Object.assign(form, {
    ...
    modules: (c.plan_flags || []).filter(f => ALL_MODULES.some(m => m.key === f.feature)).map(f => f.feature),
  })
