<template>
  <div>
    <div class="flex items-center justify-between mb-5">
      <div>
        <h2 class="text-base font-medium text-gray-900">Distribuidors</h2>
        <p class="text-sm text-gray-400 mt-0.5">{{ pagination.total }} distribuidors registrats</p>
      </div>
      <button @click="openCreate" class="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-3 py-1.5 rounded-lg transition-colors">
        <IconPlus class="w-4 h-4" />Nou distribuidor
      </button>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div v-else-if="tenants.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="text-sm text-gray-500">Encara no hi ha distribuidors</p>
        <button @click="openCreate" class="mt-3 text-sm text-blue-600 hover:underline">Crea el primer distribuidor</button>
      </div>
      <div v-else class="divide-y divide-gray-100">
        <div v-for="t in tenants" :key="t.id" class="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
          ...
          <div class="flex items-center gap-1.5 w-28 text-sm text-gray-600">
            ...
            <span>{{ t.companies_count }} {{ t.companies_count === 1 ? 'empresa' : 'empreses' }}</span>
          </div>
          <div class="flex-1 flex items-center flex-wrap gap-1.5 min-w-0">
            ...
            <span v-else class="text-xs text-gray-400 italic">Sense moduls</span>
          </div>
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            <button @click="viewTarget = t" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" title="Veure detalls"><IconEye class="w-4 h-4" /></button>
            <button @click="openEdit(t)" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Editar"><IconEdit class="w-4 h-4" /></button>
            <button @click="askDelete(t)" class="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar"><IconTrash class="w-4 h-4" /></button>
          </div>
        </div>
      </div>
      <div v-if="pagination.last_page > 1" class="flex items-center justify-between px-5 py-3 border-t bg-gray-50">
        <p class="text-xs text-gray-400">Pagina {{ pagination.current_page }} de {{ pagination.last_page }}</p>
        <div class="flex gap-1">
          <button @click="load(pagination.current_page - 1)" :disabled="pagination.current_page === 1" class="px-2.5 py-1 text-xs rounded border border-gray-200 disabled:opacity-40 hover:bg-white transition-colors">Anterior</button>
          <button @click="load(pagination.current_page + 1)" :disabled="pagination.current_page === pagination.last_page" class="px-2.5 py-1 text-xs rounded border border-gray-200 disabled:opacity-40 hover:bg-white transition-colors">Seguent</button>
        </div>
      </div>
    </div>
      ...
      <div v-if="modal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" @click.self="closeModal">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            <h3 class="font-medium text-gray-900">{{ modal.isEdit ? 'Editar distribuidor' : 'Nou distribuidor' }}</h3>
          ...
          <form @submit.prevent="submitModal" class="overflow-y-auto flex-1 px-6 py-5 space-y-6">
            ...
            <div>
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Imatges</p>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <label class="cursor-pointer inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-gray-600">
                        <IconUpload class="w-3.5 h-3.5" />Seleccionar
                        <input type="file" accept="image/*" class="hidden" @change="onLogoChange" />
                      </label>
                      <p class="text-[10px] text-gray-400 mt-1">PNG, JPG, SVG. Max 2 MB.</p>
                      <button v-if="logoPreview || form.logo_url" type="button" @click="clearLogo" class="text-[10px] text-red-500 hover:underline mt-0.5 block">Eliminar</button>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-[10px] text-gray-400 mt-1">ICO, PNG, SVG. Max 512 KB.</p>
                      <button v-if="faviconPreview || form.favicon_url" type="button" @click="clearFavicon" class="text-[10px] text-red-500 hover:underline mt-0.5 block">Eliminar</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Identificacio</p>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Identificador intern <span class="text-red-500">*</span></label>
                  <input v-model="form.nom_intern" type="text" placeholder="ex: empresa-abc"
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Nom legal</label>
                  <input v-model="form.nom_legal" type="text" placeholder="Empresa S.L."
                         class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">NIF / CIF</label>
                  <input v-model="form.nif" type="text" placeholder="B12345678"
                         class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Adreca de facturacio</label>
                  <input v-model="form.adreca_facturacio" type="text" placeholder="Carrer, num., CP, Ciutat" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Persona de contacte</label>
                  <input v-model="form.persona_contacte" type="text" placeholder="Nom i cognoms" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Telefon</label>
                  <input v-model="form.telefon" type="tel" placeholder="+34 600 000 000" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div class="col-span-2">
                  <label class="block text-xs font-medium text-gray-600 mb-1">Email de contacte</label>
                  <input v-model="form.email_contacte" type="email" placeholder="contacte@empresa.com"
                         class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Pla i configuracio</p>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Pla <span class="text-red-500">*</span></label>
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1">Maxim empleats</label>
                  ...
                  <p class="text-xs text-gray-400 mt-1">0 = illimitat</p>
                </div>
              </div>
              <div class="flex items-center gap-3 mt-4">
                ...
                <span class="text-sm text-gray-700 select-none">Distribuidor actiu</span>
              </div>
            </div>

            <div>
              <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Moduls habilitats</p>
              <div class="grid grid-cols-3 gap-3">
                <label v-for="mod in MODULES" :key="mod.key"
                  ...
                  <div>
                    ...
                    <div class="text-xs text-gray-400 mt-0.5">{{ mod.desc }}</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Superadministrador</p>
              ...
              <div v-if="form.createSuperadmin" class="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div>
                  ...
                  <input v-model="form.superadmin_name" type="text" placeholder="Nom i cognoms"
                ...
                <div>
                  ...
                  <input v-model="form.superadmin_email" type="email" placeholder="admin@empresa.com"
                ...
                <div class="col-span-2">
                  <label class="block text-xs font-medium text-gray-600 mb-1">Contrasenya <span class="text-red-500">*</span></label>
                  <input v-model="form.superadmin_password" type="password" placeholder="Minim 8 caracters"
                         class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              ...
              <p v-else class="text-xs text-gray-400">Activa aquesta opcio per crear un usuari superadministrador per a aquest distribuidor.</p>
            </div>
          ...
          <div class="flex items-center justify-end gap-2 px-6 py-4 border-t flex-shrink-0">
            <button type="button" @click="closeModal" class="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Cancel-lar</button>
            <button @click="submitModal" :disabled="saving" class="px-4 py-2 text-sm font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-60 flex items-center gap-2 transition-colors">
              ...
              {{ saving ? 'Desant...' : (modal.isEdit ? 'Desar canvis' : 'Crear distribuidor') }}
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
          <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            ...
            <div class="flex items-center gap-2">
              <button @click="openEdit(viewTarget); viewTarget = null"
                ...
                <IconEdit class="w-3.5 h-3.5" />Editar
              </button>
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div class="flex items-center gap-2">
              ...
              <span :class="viewTarget.actiu ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'" class="text-xs font-medium px-2 py-0.5 rounded-full">{{ viewTarget.actiu ? 'Actiu' : 'Inactiu' }}</span>
              <span class="text-xs text-gray-400">{{ viewTarget.companies_count }} {{ viewTarget.companies_count === 1 ? 'empresa' : 'empreses' }}</span>
              <span v-if="viewTarget.max_empleats" class="text-xs text-gray-400">· max. {{ viewTarget.max_empleats }} empleats</span>
            </div>

            <!-- Identificació -->
            <div>
              <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Identificació</p>
              <div class="grid grid-cols-2 gap-x-6 gap-y-2">
                <div v-if="viewTarget.nif">
                  <p class="text-[10px] text-gray-400">NIF / CIF</p>
                  <p class="text-sm text-gray-800">{{ viewTarget.nif }}</p>
                </div>
                <div v-if="viewTarget.adreca_facturacio">
                  <p class="text-[10px] text-gray-400">Adreça de facturació</p>
            ...
            <div v-if="viewTarget.persona_contacte || viewTarget.telefon || viewTarget.email_contacte">
              <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Contacte</p>
              <div class="grid grid-cols-2 gap-x-6 gap-y-2">
                ...
                <div v-if="viewTarget.telefon">
                  <p class="text-[10px] text-gray-400">Telèfon</p>
            ...
            <!-- Mòduls -->
            <div>
              <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Mòduls habilitats</p>
              <div v-if="activePlanModules(viewTarget).length" class="flex flex-wrap gap-1.5">
                <span v-for="mod in activePlanModules(viewTarget)" :key="mod.key" :class="mod.cls"
                      class="text-xs font-medium px-2.5 py-1 rounded-full">{{ mod.label }}</span>
              </div>
              <p v-else class="text-sm text-gray-400 italic">Cap mòdul habilitat</p>
            </div>

            <!-- Favicon -->
            <div v-if="viewTarget.whitelabel && viewTarget.whitelabel.favicon_url">
              <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Favicon</p>
              <img :src="viewTarget.whitelabel.favicon_url" class="w-8 h-8 object-contain" />
            </div>

            <!-- Data alta -->
            <div v-if="viewTarget.data_alta">
              <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1">Alta</p>
              <p class="text-sm text-gray-800">{{ new Date(viewTarget.data_alta).toLocaleDateString('ca-ES') }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="deleteTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" @click.self="deleteTarget = null">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
          <div class="flex items-start gap-3 mb-5">
            ...
            <div>
              <p class="font-medium text-gray-900">Eliminar distribuidor</p>
              <p class="text-sm text-gray-500 mt-1">Estas a punt d'eliminar <strong class="text-gray-800">{{ deleteTarget.nom_intern }}</strong>. Totes les dades s'eliminaran permanentment.</p>
            </div>
          </div>
          <div class="flex gap-2 justify-end">
            ...
            <button @click="confirmDelete" :disabled="deleting" class="px-4 py-2 text-sm font-medium bg-red-600 hover:bg-red-700 text-white rounded-lg disabled:opacity-60 transition-colors">{{ deleting ? 'Eliminant...' : 'Eliminar' }}</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const MODULES = [
  { key: 'time_tracking', label: 'Fitxatge',     desc: 'Control horari i entrades/sortides' },
  { key: 'documents',     label: 'Documentacio', desc: 'Gestio de documents i arxius' },
  { key: 'calendar',      label: 'Calendari',    desc: "Calendari d'empresa i absencies" },
]
...
function activePlanModules(t) {
  return (t.plan_feature_flags || [])
    ...
    .map(f => ({ key: f.feature, label: (MODULES.find(m => m.key === f.feature) || {}).label || f.feature, cls: MODULE_CLS[f.feature] || 'bg-gray-100 text-gray-600' }))
}
